...
Download `Glance-Plex-v1.0.0-Setup.exe` from [the latest release](https://github.com/Brusko25/Glance-Plex-Releases/releases/latest). Run it and choose the optional desktop shortcut or Windows startup tasks if wanted. Installation is per-user and does not require administrator rights. The installer and application are unsigned, so Windows may display an unknown-publisher prompt.
